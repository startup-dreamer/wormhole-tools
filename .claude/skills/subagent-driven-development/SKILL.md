...
|-----------|-------|----------|
| **Rust** | `rust-developer` | SDK modules, CLI commands, VAA parsing, chain integrations, async/await, error types, clap commands |
| **Shell** | `bash-script-craftsman` | Build scripts, CI helpers, shell tooling |
| **Mixed** | Split into subtasks | If a task has both Rust and shell work, split it and dispatch to appropriate agents |

**Decision criteria:**
- Does the task involve Rust code? → `rust-developer`
- Does the task involve shell scripting? → `bash-script-craftsman`
- Does the task touch both? → Split it, Rust first, then shell glue
